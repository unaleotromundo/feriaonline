...
<<<<<<< HEAD
    ...
    const changeMessage = () => {
        if (loadingMessageElement) {
            loadingMessageElement.textContent = appLoadingMessages[messageIndex % appLoadingMessages.length];
            messageIndex++;
        }
    };
    changeMessage();
    ...
    // Simulación de carga inicial (puedes ajustar el tiempo)
    setTimeout(() => {
        clearInterval(intervalId);
        hideGlobalLoadingOverlay();
        // Mostrar la sección principal
        showSection('products');
    }, 4000 * appLoadingMessages.length);
}
